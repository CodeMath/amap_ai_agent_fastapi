...
def test_get_agent_start_not_found(client: TestClient):
    response = client.get("/api/agents/start/nonexistent-agent-id-12345")
...
def test_register_agent_invalid_payload(client: TestClient):
    response = client.post(
        "/api/agents/register", json={"name": "only name, missing fields"}
    )
...
def test_get_registered_agent_after_registration(client: TestClient):
    ...
    ), f"Registration failed: {post_response.text}"
    ...
    # Decimal 타입 변환 문제 해결: float로 변환하여 비교
    assert float(response_data["latitude"]) == float(payload_for_this_test["latitude"])
    assert float(response_data["longitude"]) == float(
        payload_for_this_test["longitude"]
    )


AGENT_ID_FOR_UPDATE_PROMPT_TEST = (
    "agent-for-update-prompt-test-001"  # These can remain fixed if helpers manage state
)
AGENT_FOR_UPDATE_PROMPT_PAYLOAD = {
    "agent_id": AGENT_ID_FOR_UPDATE_PROMPT_TEST,
    "name": "UpdatePromptAgent",
    "description": "Agent for testing prompt updates.",
    "prompt": "Initial prompt.",
    "thumbnail": "http://example.com/thumb_update.png",
    "tools": [],
    "model": "gpt-3",
    "latitude": 50.0,
    "longitude": 60.0,
...
def test_update_agent_prompt_success(client: TestClient):
    ...
    reg_response = client.post(
        "/api/agents/register", json=AGENT_FOR_UPDATE_PROMPT_PAYLOAD
    )
    if (
        reg_response.status_code == 500
        and "already exists" in reg_response.text.lower()
    ):
        ...
        ), f"Failed to register agent for update test: {reg_response.text}"

    update_data = {"prompt": "New updated prompt content"}
    response = client.put(
        f"/api/agents/{AGENT_ID_FOR_UPDATE_PROMPT_TEST}/update-prompt", json=update_data
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_update_agent_prompt_not_found(client: TestClient):
    response = client.put(
        "/api/agents/nonexistentagent123abc/update-prompt",
        json={"prompt": "Doesn't matter"},
    )
    assert response.status_code == 500
...
def test_update_agent_prompt_invalid_payload(client: TestClient):
    ...
    if (
        ...
        assert (
            reg_response.status_code == 200
        ), f"Failed to register agent for invalid payload update test: {reg_response.text}"

    response = client.put(
        f"/api/agents/{AGENT_ID_FOR_UPDATE_PROMPT_TEST}/update-prompt",
...
AGENT_TO_DELETE_PAYLOAD = {
    ...
    "name": "DeleteMeAgent",
    "description": "Agent specifically for delete testing.",
    "prompt": "Delete me.",
    "thumbnail": "http://example.com/thumb_delete.png",
    "tools": [],
    "model": "gpt-3",
    "latitude": 70.0,
    "longitude": 80.0,
...
AGENT_FOR_ACHIEVEMENTS_PAYLOAD = {
    "agent_id": AGENT_ID_FOR_ACHIEVEMENTS_TEST,
    "name": "AchievementsAgent",
    "description": "Agent for testing adding achievements.",
    "prompt": "Achieve things.",
    "thumbnail": "http://example.com/thumb_ach.png",
    "tools": [],
    "model": "gpt-3",
    "latitude": 90.0,
    "longitude": 100.0,
...
def test_add_achievements_to_agent_success(client: TestClient):
    # Register agent for this test
    reg_response = client.post(
        "/api/agents/register", json=AGENT_FOR_ACHIEVEMENTS_PAYLOAD
    )
    if (
        reg_response.status_code == 500
        and "already exists" in reg_response.text.lower()
        ...
        assert (
            reg_response.status_code == 200
        ), f"Failed to register agent for achievements test: {reg_response.text}"

    response = client.post(
        f"/api/agents/{AGENT_ID_FOR_ACHIEVEMENTS_TEST}/add-achievements",
        json=[TEST_ACHIEVEMENT_PAYLOAD],
    )
    assert response.status_code == 200
...
def test_add_achievements_to_agent_not_found(client: TestClient):
    response = client.post(
        "/api/agents/nonexistentagent-for-ach-123/add-achievements",
        json=[TEST_ACHIEVEMENT_PAYLOAD],
    )
    assert response.status_code == 500
...
def test_add_achievements_invalid_payload(client: TestClient):
    ...
    if (
        ...
        assert (
            reg_response.status_code == 200
        ), f"Failed to register agent for invalid achievement payload test: {reg_response.text}"

    response = client.post(
        f"/api/agents/{AGENT_ID_FOR_ACHIEVEMENTS_TEST}/add-achievements",
        json=[{"wrong_field": "xyz"}],
    )
    assert response.status_code == 422
...
AGENT_FOR_CHAT_TESTS_PAYLOAD = {
    "agent_id": AGENT_ID_FOR_CHAT_TESTS,
    "name": "ChatHistoryTestAgent",
    "description": "Agent for testing chat history endpoints.",
    "prompt": "Let's chat.",
    "thumbnail": "http://example.com/thumb_chat.png",
    "tools": [],
    "model": "gpt-3",
    "latitude": 10.1,
    "longitude": 20.2,
...
def register_chat_test_agent(client: TestClient):
    reg_response = client.post(
        "/api/agents/register", json=AGENT_FOR_CHAT_TESTS_PAYLOAD
    )
    if (
        reg_response.status_code == 500
        and "already exists" in reg_response.text.lower()
        ...
        assert (
            reg_response.status_code == 200
        ), f"Failed to register agent for chat tests: {reg_response.text}"
...
AGENT_FOR_STREAM_TESTS_PAYLOAD = {
    "agent_id": AGENT_ID_FOR_STREAM_TESTS,
    "name": "StreamTestAgent",
    "description": "Agent for testing run-stream endpoint.",
    "prompt": "Stream something.",
    "thumbnail": "http://example.com/thumb_stream.png",
    "tools": ["some_tool"],
    "model": "gpt-3.5-turbo",  # Assuming tools can be simple strings
    "latitude": 12.3,
    "longitude": 45.6,
    "achievements": [],
}
...
def register_stream_test_agent(client: TestClient):
    reg_response = client.post(
        "/api/agents/register", json=AGENT_FOR_STREAM_TESTS_PAYLOAD
    )
    if (
        reg_response.status_code == 500
        and "already exists" in reg_response.text.lower()
        ...
        assert (
            reg_response.status_code == 200
        ), f"Failed to register agent for stream tests: {reg_response.text}"


def test_run_stream_success(client: TestClient):
    register_stream_test_agent(client)
    agent_request_payload = AgentRequestDTO(
        agent_id=AGENT_ID_FOR_STREAM_TESTS,
        data="Hello agent for stream",
        response_id=None,
    )

    response = client.post(
        "/api/agents/run-stream", json=agent_request_payload.model_dump()
    )
...
def test_run_stream_agent_not_found(client: TestClient):
    agent_request_payload = AgentRequestDTO(
        agent_id="nonexistent-stream-agent-123", data="Hello", response_id=None
    )
    response = client.post(
        "/api/agents/run-stream", json=agent_request_payload.model_dump()
    )
...
def test_run_stream_invalid_payload(client: TestClient):
    response = client.post(
        "/api/agents/run-stream", json={"wrong_field": "xyz"}
    )  # Missing agent_id and data
    assert response.status_code == 422
...
AGENT_FOR_GEN_ACH_TESTS_PAYLOAD = {
    "agent_id": AGENT_ID_FOR_GEN_ACH_TESTS,
    "name": "GenAchTestAgent",
    "description": "Agent for testing generate-achievements endpoint.",
    "prompt": "Generate achievements for me.",
    "thumbnail": "http://example.com/thumb_gen_ach.png",
    "tools": [],
    "model": "gpt-4o-mini",  # OpenAI API 호환성을 위해 gpt-4o-mini로 변경
    "latitude": 78.9,
    "longitude": 101.1,
    "achievements": [],
}
...
def register_gen_ach_test_agent(client: TestClient):
    reg_response = client.post(
        "/api/agents/register", json=AGENT_FOR_GEN_ACH_TESTS_PAYLOAD
    )
    if (
        reg_response.status_code == 500
        and "already exists" in reg_response.text.lower()
        ...
        assert (
            reg_response.status_code == 200
        ), f"Failed to register agent for generate achievement tests: {reg_response.text}"


def test_generate_achievements_success(client: TestClient):
    register_gen_ach_test_agent(client)
    response = client.post(
        f"/api/agents/{AGENT_ID_FOR_GEN_ACH_TESTS}/generate-achievements"
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_generate_achievements_agent_not_found(client: TestClient):
    response = client.post(
        "/api/agents/nonexistent-gen-ach-agent-123/generate-achievements"
    )
    assert response.status_code == 500
