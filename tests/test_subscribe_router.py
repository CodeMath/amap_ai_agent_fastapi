...
from app.agents.schemas.user_schemas import SubscriptionIn # Correct schema for request body
...
def test_subscribe_success(client: TestClient):
    ...
    """
    subscription_data = {
        "endpoint": "https://example.com/push-endpoint/123unique",
        "keys": {
...
def test_subscribe_invalid_payload_missing_keys(client: TestClient):
    ...
    """
    invalid_data = {
        "endpoint": "https://example.com/push-endpoint/anotherone"
    }
    response = client.post("/api/push/subscribe", json=invalid_data)
